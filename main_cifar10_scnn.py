...
if __name__ == '__main__':
    ...
    cnn = Cifar10Net(input_shape=(32, 32, 3), num_classes=10)
    cnn = cnn.cuda() if simulation_config['use_gpu'] else cnn

    optimizer = torch.optim.Adam(cnn.parameters(), lr=0.005)
    lr_sch = lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.5)
    criterion = torch.nn.CrossEntropyLoss()

    # for i in range(20):
    #     train_ann(model=cnn, train_loader=train_loader, optimizer=optimizer, criterion=criterion, epoch=i + 1,
    #           simulation_config=simulation_config)
    #
    #     test_ann(model=cnn, test_loader=test_loader, criterion=criterion, simulation_config=simulation_config)
    #     lr_sch.step()
    # torch.save(cnn.state_dict(), 'model_output/Cifar10Net_params.pkl')

    device = torch.device('cuda')
    scnn = SpikingCifar10Net(input_shape=(32, 32, 3), num_classes=10, if_param=IFParameters(), device=device,
    ...
    scnn.load_state_dict(torch.load('model_output/Cifar10Net_params.pkl'))
